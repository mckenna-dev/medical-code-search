...
class Command(BaseCommand):
    help = 'Bulk download all GitHub codelists for manual standardization'

    def add_arguments(self, parser):
        parser.add_argument('--metadata-csv', type=str, required=True, 
                          help='Path to CSV file containing codelist metadata')
        parser.add_argument('--download-root', type=str, default='./bulk_download/',
    ...
    def handle(self, *args, **options):
        ...
        for index, row in df.iterrows():
            result = self.process_download(row, download_path, dry_run, resume)
        ...
        self.generate_summary_report(download_path, summary, dry_run)
        
        # Generate standardization template
        if not dry_run:
            self.generate_standardization_template(download_path)

    def process_download(self, row, download_path, dry_run, resume):
        ...
        if resume and file_path.exists():
            self.stdout.write(f'SKIP: {folder_name} (already exists)')
            return {'status': 'skipped', 'project': safe_project}
        
        self.stdout.write(f'Processing: {folder_name}')
        
        if dry_run:
            self.stdout.write(f'  Would download: {github_url}')
            self.stdout.write(f'  To: {file_path}')
        ...
        if download_result['success']:
            ...
            self.stdout.write(f'  SUCCESS: Downloaded {analysis.get("rows", "?")} rows')
            return {'status': 'success', 'project': safe_project}
        else:
            ...
            self.stdout.write(f'  FAILED: {download_result.get("error", "Unknown error")}')
    ...
    def download_file(self, github_url, file_path, metadata_path, row):
        ...
        try:
            ...
            if 'text/html' in content_type:
                return {'success': False, 'error': 'Received HTML (likely error page)'}
    ...
    def analyze_downloaded_file(self, file_path):
        ...
        try:
            ...
            # Get sample data
            sample = df.head(3).to_string() if len(df) > 0 else "No data"
    ...
    def generate_summary_report(self, download_path, summary, dry_run):
        """Generate a summary report"""
        
        self.stdout.write(f'\n=== DOWNLOAD SUMMARY ===')
        self.stdout.write(f'Total entries: {summary["total"]}')
        self.stdout.write(f'Successful downloads: {summary["successful"]}')
        ...
        for project, count in sorted(summary['projects'].items()):
            self.stdout.write(f'  {project}: {count} codelists')
    ...
    def generate_standardization_template(self, download_path):
        ...
        self.stdout.write(f'\nGenerated standardization template: {template_path}')
...
""")
        ...
        self.stdout.write(f'Generated README: {readme_path}')